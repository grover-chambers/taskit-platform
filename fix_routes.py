...
# 1. Clean up the conflicting route groups from previous iterations
conflicting_folders = ['src/app/(customer)', 'src/app/(dashboard)', 'src/app/(rider)']
for folder in conflicting_folders:
    if os.path.exists(folder):
        shutil.rmtree(folder)
        print(f"Deleted conflicting folder: {folder}")

# 2. Define the clean, flat file structure
files = {
...
""",
    "src/app/rider/page.tsx": """export default function RiderDashboard() {
...
""",
    "src/app/book/page.tsx": """"use client";
...
print("\nClean flat structure built successfully!")
