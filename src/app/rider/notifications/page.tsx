...
export default function RiderNotifications() {
  ...
  const handleNotifClick = (notif: any) => {
    ...
    if (notif.type === 'ORDER' && notif.body) {
      const match = notif.body.match(/#([A-Z0-9]{7})/);
      if (match) {
        router.push(`/rider`);
      }
    }
        ...
        <div className="relative space-y-4">
          ...
          {notifications.map((notif) => (
            <div
              ...
              className="relative flex items-start space-x-4 pl-6 cursor-pointer"
            >
              <div className={`absolute left-0 top-2 w-6 h-6 rounded-full flex items-center justify-center border-2 ${
                notif.read
                  ? 'border-midnight-700 bg-midnight-800'
                  : 'border-gold-500 bg-midnight-950 shadow-[0_0_10px_rgba(212,175,55,0.2)]'
              }`}>
                {!notif.read && <div className="w-2 h-2 bg-gold-500 rounded-full" />}
              </div>

              <div className={`flex-1 p-4 rounded-2xl border shadow-soft-dark ${
                notif.read
                  ? 'bg-midnight-800 border-midnight-700 opacity-70'
